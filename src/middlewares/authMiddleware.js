...
export async function authMiddleware(req, res, next) {
    ...
    if(parts.length !== 2) return res.status(401).send({message: 'Invalid token 5'});

    const [schema, token] = parts;
    if (!/^Bearer$/i.test(schema)) return res.status(401).send({message: 'Invalid token'});

    jwt.verify(token, process.env.SECRET, async(err, decode) =>{
        if(err) return res.status(401).send({message: 'Invalid toke'});
        if(!decode) return res.status(401).send({message: 'Invalid token2'});
